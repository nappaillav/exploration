...
#     def make_env(env_id: str, seed: int, run_name='Test') -> Callable:
#         def _thunk():
...
def make_env(env_id: str = "SuperMarioBros-v0",
            seed: int = 0,
            gray_scale: bool = False,
            frame_stack: int = 0):
    def _thunk():
...
class RecordEpisodeStatistics(gym.Wrapper):
    def __init__(self, env, deque_size=100):
        ...
        self.list_return = deque(maxlen=deque_size)
        self.list_length = deque(maxlen=deque_size)
    ...
    def step(self, action):
        # print("Value : {}".format(len(super().step(action))))
        # print(super().step(action))
        ...
        if np.sum(dones):
            self.list_return.extend(list(self.returned_episode_returns[dones]))
            self.list_length.extend(list(self.returned_episode_lengths[dones])) 
        infos["r"] = self.returned_episode_returns
        infos["l"] = self.returned_episode_lengths
        infos['avg_return'] = np.mean(self.list_return) if len(self.list_return) else 0.0
        infos['avg_len'] = np.mean(self.list_length) if len(self.list_length) else 0.0
        return (
            observations,
            rewards,
            dones,
...
if __name__ == "__main__":
    ...
    envs = RecordEpisodeStatistics(envs) # See if its required with your environment 
    ...
    for step in tqdm(range(10 * args.num_iterations_obs_norm_init)):
        acs = np.random.randint(0, envs.single_action_space.n, size=(args.num_envs,))
        # print("Value - internal : {}".format(len(envs.step(acs))))
        s, r, d, info = envs.step(acs)
        next_ob += s[:, 0, :, :].reshape([-1, 1, 84, 84]).tolist()

        if len(next_ob) % (args.num_steps) == 0:
            next_ob = np.stack(next_ob)
            obs_rms.update(next_ob)
            next_ob = []
    print("End to Normalization... \nRandom Policy: Average_return : {}| Average length : {}".format(info['avg_return'], info["avg_len"]))
    ...
    for update in tqdm(range(1, num_updates + 1)):
        ...
        for step in range(0, args.num_steps):    
            ...
            rnd_next_obs = (
                (
                    (next_obs[:, 0, :, :].reshape(args.num_envs, 1, 84, 84) - torch.from_numpy(obs_rms.mean).to(device))
                    / torch.sqrt(torch.from_numpy(obs_rms.var).to(device))
                ).clip(-5, 5)
            ).float()
            # Should there be a torch.no_grad()
            target_next_feature = rnd_model.target(rnd_next_obs)
            predict_next_feature = rnd_model.predictor(rnd_next_obs)
            curiosity_rewards[step] = ((target_next_feature - predict_next_feature).pow(2).sum(1) / 2).data
            for idx, d in enumerate(done):
                if d and info["life"][idx] == 0:
        ...
        obs_rms.update(b_obs[:, 0, :, :].reshape(-1, 1, 84, 84).cpu().numpy())
        ...
        rnd_next_obs = (
            (
                (b_obs[:, 0, :, :].reshape(-1, 1, 84, 84) - torch.from_numpy(obs_rms.mean).to(device))
                / torch.sqrt(torch.from_numpy(obs_rms.var).to(device))
            ).clip(-5, 5)
        ).float()
